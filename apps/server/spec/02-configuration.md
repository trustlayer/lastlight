...
interface ReviewAnalysisConfig {          // docs/plans/deterministic-pr-levers.md
  ...
  maxInlineComments: number;              // attention boundary — a budget, not a filter; 5 (issue #405, was 10), spent on DERIVED severity
...
The `post-review` phase adds one in-process key, `review-summary` (issue #405):
under the evidence pipeline the posted review summary is written by one
tool-free model call AFTER the attention boundary has decided what posts, from
the posted findings only (`src/engine/github/review-summary.ts`). It resolves
...
to a summary rendered in code, and the adjudicator's own `summary` is never
